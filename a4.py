# a4.py
# Jeffrey Zhang (jz674) and Nicolas Barone (njb227)
# 10/26/16
...
def draw_two_lines(w,sp):
    ...
    t.speed = sp
    t.color = 'green'
    t.forward(100.0) # draw a line 100 pixels in the current direction
    t.left(90.0)     # add 90 degrees to the angle
    t.color = 'red'
    t.forward(200.0)
...
def draw_spiral(w, side, ang, n, sp):
    ...
    assert is_valid_length(side), (report_error('side is not a valid'+
        'length',side))
    assert is_valid_iteration(n), (report_error('n is not a valid number'+
        'of iterations',side))
    assert is_valid_speed(sp), report_error('sp is not a valid speed',sp)
    assert is_number(ang), report_error('angle is not number')
...
def multi_polygons(w, side, k, n, sp):
    ...
    assert is_valid_length(side), (report_error('side is not a valid' +
        'length',side))
    assert is_valid_speed(sp), report_error('sp is not a valid speed',sp)
    assert is_valid_iteration(k), report_error('k is not an int >= 1')
    assert (type(n) == int and 3 <=n), report_error(' n is not an int >= 3')
...
def draw_polygon(t, side, n, sp):
    ...
    assert is_valid_length(side), (report_error('side is not a valid'
        +'length',side))
    assert (type(n) == int and n >= 3), (report_error('n is an invalid # of'
        +'poly sides',n))
    assert is_valid_speed(sp), report_error('sp is not a valid speed',sp)
...
def radiate(w, side, n, sp):
    ...
    assert is_valid_length(side), (report_error('side is not a valid'
        + 'length',side))
    assert is_valid_speed(sp), report_error('sp is not a valid speed',sp)
    assert (type(n) == int and 2 <=n), report_error(' n is not an int >= 2')
...
def cantor(w, side, hght, d):
    ...
    assert is_valid_length(side), (report_error('side is not a valid'
        + 'length',side))
    assert is_valid_length(hght), (report_error('hght is not a valid'
        +'length', hght))
    assert is_valid_depth(d), report_error('d is not an int >= 0')
...
def cantor_helper(p, x, y, side, hght, d):
    ...
    assert is_valid_length(hght), (report_error('hght is not a valid'
        + 'length',hght))
    assert is_valid_depth(d), report_error('d is not an int >= 0')
    assert is_number(x), report_error('x is not a number')
    assert is_number(y), report_error('y is not a number')
...
def sierpinski(w, side, d):
    ...
    assert is_valid_length(side), report_error('side is not a valid'
        +'length',side)
    assert is_valid_depth(d), report_error('d is not an int >= 0 or depth')
...
def sierpinski_helper(p, x, y, side, d):
    """Draws a Sierpinski triangle with side length and depth d anchored at
    (x, y).
    ...
    if (d==0):
        ...
        p.move(x+side/4.0, y+side/2.0 * math.sqrt(0.75))
        
        p.move(x + side/2.0, y)    
        sierpinski_helper(p, x + side/2.0, y, side/2.0, (d-1))

        (sierpinski_helper(p, x+side/4.0, y + side/2.0 * math.sqrt(0.75),
            side/2.0, (d-1)))
...
def branches(w, hght, d):
    ...
    # ARE THESE ALL OF THE PRECONDITIONS?
    assert is_window(w), report_error('w is not a valid window',w)
    assert is_valid_length(hght), report_error('hght is not a'
        +'valid length',hght)
    assert is_valid_depth(d), report_error('d is not an int >= 0')
